...
private const val DB_NAME = "cryptonews.db"
...
private val CREATE = "create table $ITEMS(" +
        ...
        "${Item::articleUrl.name} text not null, " +
        "${Item::description.name} text not null, " +
        "${Item::picturePath.name} text not null, " +
        "${Item::date.name} text not null, " +
        "${Item::provider.name} text not null, " +
        "${Item::favorite.name} integer not null " +
        ")"


private const val DROP = "drop table $ITEMS"

class CryptoNewsSqlHelper(
    context: Context?

) : SQLiteOpenHelper(context, DB_NAME, null, DB_VERSION), CryptoNewsRepository {
